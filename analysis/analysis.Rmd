...
```{r}
library(dplyr)
difference <- data.frame(variable=header[-1], max_diff=NA)
index <- 1
for(col in header[-1]){
  print(col)
  ...
  #print(pairwise.wilcox.test(wine[[col]], wine$type, p.adjust.method = "holm"))
  index <- index + 1
}
...
Finalment escriurem les dades que tenim en el dataset en un fitxer CSV. Hi afegirem els valors obtinguts de cada algorsimes de classificació juntament amb la classificació original en els casos dels algorismes no supervisats.
